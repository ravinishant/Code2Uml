...
public class MouseDragListener implements MouseListener, MouseMotionListener {
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        int oldX = c.getX();
        int oldY = c.getY();
        c.setLocation(Math.max(0, Math.min(c.getParent().getWidth() - c.getWidth(), c.getX() + e.getX() - clickX)),
                      Math.max(0, Math.min(c.getParent().getHeight() - c.getHeight(), c.getY() + e.getY() - clickY)));
